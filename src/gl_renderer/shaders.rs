...
pub fn compile_basic_program<GL: HasContext>(gl: &Rc<RefCell<GL>>,
                                             frag_src: &str,
                                             vert_src: &str) ->
                                             Result<resources::Program<GL>, resources::GlResourceError> {
  compile_program(gl, &[(glow::FRAGMENT_SHADER, frag_src), (glow::VERTEX_SHADER, vert_src)])
}

pub fn compile_program<GL: HasContext>(gl: &Rc<RefCell<GL>>, sources: &[(u32, &str)]) ->
Result<resources::Program<GL>, resources::GlResourceError> {
  ...
  {
    let gl = gl.borrow();
    let iter: Vec<_> = shaders.shaders().iter().zip(sources).map(|(shader, source)| (shader, source.1)).collect();
    ...
    if !unsafe { gl.get_program_link_status(*program.program()) } {
      ...
      log::error!("vs compile status: {:?}", unsafe { gl.get_shader_compile_status(vs)});
      log::error!("fs compile status: {:?}\n{}", unsafe { gl.get_shader_compile_status(fs)}, fs_info_log);
      return Err(GlResourceError::new(format!("program link error: {}\n", info_log), None));
    }
  }
