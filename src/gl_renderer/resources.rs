...
impl fmt::Display for GlResourceError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "GlResourceError {}, gl error code: {:?}", &self.reason, self.gl_code)
    }
}
...
impl From<Vec<(u32, String)>> for GlResourceError {
    fn from(v: Vec<(u32, String)>) -> Self {
        ...
        let reasons: Vec<String> = v.into_iter().map(|(a, b)| format!("{}, {}", a, b)).collect();
...
impl<GL: HasContext> Shaders<GL> {
    pub fn create(gl: &Rc<RefCell<GL>>, shader_types: &[u32]) -> Result<Self, GlResourceError> {
        let mut resource = Self { gl: Rc::downgrade(gl), shaders: smallvec![] };
        {
            let gl = gl.borrow();
            for &shader_type in shader_types {
                unsafe {
                    let shader = gl.create_shader(shader_type).map_err(
                        |e| GlResourceError::new(e, None)
                    )?;
...
impl<GL: HasContext> Program<GL> {
    pub fn create(gl: &Rc<RefCell<GL>>) -> Result<Self, GlResourceError> {
        ...
        Ok(Self{program, gl: Rc::downgrade(gl)})
    }
