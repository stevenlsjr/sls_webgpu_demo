...
use std::rc::Rc;
use super::resources::{self, GlResourceError};
use super::shaders;
...
pub mod html5 {
    ...
    impl FromCanvas for GlContext {
        fn from_canvas(canvas: HtmlCanvasElement) -> Result<Self, crate::Error> {
            ...
            let main_program = shaders::compile_basic_program(&gl,
    ...
    impl GlContext {
        ...
        pub fn set_clear_color(&mut self, color: Vec4) {
            ...
            unsafe {
                self.gl.borrow().clear_color(self.clear_color.x, self.clear_color.y, self.clear_color.z, self.clear_color.w);
            }
        }
    }
}
